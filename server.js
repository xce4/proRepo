...
app.get("/socket", (req, res, next) => {
    res.send(`<div id="add"></div></ul>
<script src="/socket.io/socket.io.js"></script>
<script>
  var socket = io();
  socket.on('message',(m)=>{
      document.getElementById("add").innerText=m;
  })
</script>`)
});
...
app.get("/second", async (req, res, next) => {
    ...
    exec("python3 install.py>>log.txt", function (err, stdout, stderr) {
    });
    var time = Number(fs.readFileSync("last.txt"));
    if ((time + 86400000) > Date.now()) res.json({error: "Come back tomorrow"});
